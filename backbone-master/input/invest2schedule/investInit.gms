...
* --- Define Key Execution Parameters in Time Indeces -------------------------
    ...
    mSettings('invest', 't_end') = 8736+1; // Last time step to be included in the solve (may solve and output more time steps in case t_jump does not match)

    // Define simulation horizon and moving horizon optimization "speed"
    mSettings('invest', 't_horizon') = 8736+1;    // How many active time steps the solve contains (aggregation of time steps does not impact this, unless the aggregation does not match)
    mSettings('invest', 't_jump') = 8736+1;          // How many time steps the model rolls forward between each solve

    // Define 168 of data for proper circulation
    mSettings('invest', 'dataLength') = 8736;
* =============================================================================
...
* --- Define Samples ----------------------------------------------------------
	...
	msStart('invest', 's000') = 7057;
	msEnd('invest', 's000') = 7225;
	msStart('invest', 's001') = 4537;
	msEnd('invest', 's001') = 4705;
	msStart('invest', 's002') = 673;
	msEnd('invest', 's002') = 841;
	msStart('invest', 's003') = 4201;
	msEnd('invest', 's003') = 4369;
	msStart('invest', 's004') = 6385;
	msEnd('invest', 's004') = 6553;
	...
	p_msWeight('invest', 's000') = 12.032967;
	p_msWeight('invest', 's001') = 10.027473;
	p_msWeight('invest', 's002') = 13.035714;
	p_msWeight('invest', 's003') = 13.035714;
	p_msWeight('invest', 's004') = 4.010989;

    p_msAnnuityWeight('invest', s) = 0;
    p_msAnnuityWeight('invest', 's000') = 2021.538462/8760;
    p_msAnnuityWeight('invest', 's001') = 1684.615385/8760;
    p_msAnnuityWeight('invest', 's002') = 2190.000000/8760;
    p_msAnnuityWeight('invest', 's003') = 2190.000000/8760;
    p_msAnnuityWeight('invest', 's004') = 673.846154/8760;
