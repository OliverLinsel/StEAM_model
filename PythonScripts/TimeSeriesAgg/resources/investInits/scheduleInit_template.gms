...
* --- Define Key Execution Parameters in Time Indeces -------------------------
    ...
    mSettings('invest', 't_end') = complete_length; // Last time step to be included in the solve (may solve and output more time steps in case t_jump does not match)

    // Define simulation horizon and moving horizon optimization "speed"
    mSettings('invest', 't_horizon') = complete_length;    // How many active time steps the solve contains (aggregation of time steps does not impact this, unless the aggregation does not match)
    mSettings('invest', 't_jump') = 1752 ; //8760;          // How many time steps the model rolls forward between each solve

    // Define length of data for proper circulation
    mSettings('invest', 'dataLength') = complete_length;
...
* --- Define Samples ----------------------------------------------------------
    ...
    msEnd('invest', 's000') = msStart('invest', 's000')+complete_length+mSettings('invest', 't_horizon');
    ...
    p_msAnnuityWeight('invest', 's000') = complete_length/8760;
...
* --- Define Time Step Intervals ----------------------------------------------
    ...
    mInterval('invest', 'lastStepInIntervalBlock', 'c000') = 2472; // mSettings('invest', 't_horizon'); // 60;
    mInterval('invest', 'stepsPerInterval', 'c001') = 6;
    mInterval('invest', 'lastStepInIntervalBlock', 'c001') = 3504;
    mInterval('invest', 'stepsPerInterval', 'c002') = 24;
    mInterval('invest', 'lastStepInIntervalBlock', 'c002') = 5256;
    mInterval('invest', 'stepsPerInterval', 'c003') = 73;
    mInterval('invest', 'lastStepInIntervalBlock', 'c003') = 7008;
    mInterval('invest', 'stepsPerInterval', 'c004') = 292;
    mInterval('invest', 'lastStepInIntervalBlock', 'c004') = 8760;
    mInterval('invest', 'stepsPerInterval', 'c005') = 8760;
    mInterval('invest', 'lastStepInIntervalBlock', 'c005') =  mSettings('invest', 't_horizon');
