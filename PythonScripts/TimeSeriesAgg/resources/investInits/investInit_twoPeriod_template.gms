...
* --- Define Key Execution Parameters in Time Indeces -------------------------
    ...
    mSettings('invest', 't_end') = lastTimeStep+1; // Last time step to be included in the solve (may solve and output more time steps in case t_jump does not match)

    // Define simulation horizon and moving horizon optimization "speed"
    mSettings('invest', 't_horizon') = lastTimeStep+1;    // How many active time steps the solve contains (aggregation of time steps does not impact this, unless the aggregation does not match)
    mSettings('invest', 't_jump') = lastTimeStep+1;          // How many time steps the model rolls forward between each solve
...
* --- Define Samples ----------------------------------------------------------
    ...
    msStart('invest', 's000') = sample0_start;
    msEnd('invest', 's000') = sample0_start+period_length;
    msStart('invest', 's001') = sample1_start;
    msEnd('invest', 's001') = sample1_start+period_length;
    ...
    p_msWeight('invest', 's000') = weight_1;
    p_msWeight('invest', 's001') = weight_2;
    p_msAnnuityWeight('invest', s) = 0;
    p_msAnnuityWeight('invest', 's000') = period_length*weight_1/8760;
    p_msAnnuityWeight('invest', 's001') = period_length*weight_2/8760;
...
* --- Define Time Step Intervals ----------------------------------------------
    ...
    mInterval('invest', 'lastStepInIntervalBlock', 'c000') = lastTimeStep+1;
