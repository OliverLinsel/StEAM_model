...
* --- Define Key Execution Parameters in Time Indeces -------------------------
    ...
    mSettings('invest', 't_end') = completeTSlen+1; // Last time step to be included in the solve (may solve and output more time steps in case t_jump does not match)

    // Define simulation horizon and moving horizon optimization "speed"
    mSettings('invest', 't_horizon') = completeTSlen+1;    // How many active time steps the solve contains (aggregation of time steps does not impact this, unless the aggregation does not match)
    mSettings('invest', 't_jump') = completeTSlen+1;          // How many time steps the model rolls forward between each solve

    // Define length of data for proper circulation
    mSettings('invest', 'dataLength') = completeTSlen;
* =============================================================================
...
* --- Define Samples ----------------------------------------------------------
    ...
    mSettings('invest', 'samples') = numberOfPeriods;
...
* --- Define Time Step Intervals ----------------------------------------------
    ...
    mInterval('invest', 'stepsPerInterval', 'c000') = hoursPerTimeStep;
    mInterval('invest', 'lastStepInIntervalBlock', 'c000') = completeTSlen+1;

* --- z-structure for superpositioned nodes ----------------------------------

    // number of candidate periods in model
    // please provide this data
    mSettings('invest', 'candidate_periods') = num_number_candidate_periods;//0;
...
//);
    ...
    loop(gnu_output(grid, node, unit),
        if(p_gnu_io(grid, node, unit, 'input', 'conversionCoeff')  = replace_this_with_EPS,
           p_gnu_io(grid, node, unit, 'input', 'conversionCoeff') = Eps;
          ) ;
        if(p_gnu_io(grid, node, unit, 'output', 'capacity')  = replace_this_with_EPS,
           p_gnu_io(grid, node, unit, 'output', 'capacity') = Eps;
          ) ;
        if(p_gnu(grid, node, unit,  'capacity')  = replace_this_with_EPS,
           p_gnu(grid, node, unit,'capacity') = Eps;
          ) ;
        if(p_gnu(grid, node, unit,  'conversionCoeff')  = replace_this_with_EPS,
