...
* --- Define Key Execution Parameters in Time Indeces -------------------------
    ...
    mSettings('invest', 't_end') = period_end+1; // Last time step to be included in the solve (may solve and output more time steps in case t_jump does not match)

    // Define simulation horizon and moving horizon optimization "speed"
    mSettings('invest', 't_horizon') = period_end+1;    // How many active time steps the solve contains (aggregation of time steps does not impact this, unless the aggregation does not match)
    mSettings('invest', 't_jump') = period_end+1;          // How many time steps the model rolls forward between each solve
...
* --- Define Samples ----------------------------------------------------------
    ...
    msStart('invest', 's000') = period_start;
    msEnd('invest', 's000') = period_end+1;

    // Define the probability (weight) of samples
    p_msProbability('invest', s) = 0;
    p_msProbability('invest', 's000') = 1;
    p_msWeight('invest', s) = 0;
    p_msWeight('invest', 's000') = totalNumberOfPeriods;
    p_msAnnuityWeight('invest', s) = 0;
    p_msAnnuityWeight('invest', 's000') = totalNumberOfPeriods*(period_end-period_start+1)/8760;
...
* --- Define Time Step Intervals ----------------------------------------------
    ...
    mInterval('invest', 'lastStepInIntervalBlock', 'c000') = period_end+1;
