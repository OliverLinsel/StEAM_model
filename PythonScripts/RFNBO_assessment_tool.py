...
try:
    ...
    path_RFNBO_paper_vis            = r"..\Data\HPC_results\RFNBO\figures"
    path_RFNBO_paper_res            = r"..\Data\HPC_results\RFNBO"
except: 
...
prerun_path = os.path.join(path_RFNBO_paper_res, "2_Def_Grid\\2_Def_Grid_prerun")
...
r_gen_gn_df = r_gen_gn_df.drop(columns=["node", "grid"])
r_gen_gn_df = r_gen_gn_df.rename(columns={"Val":"el generation [MWh]"})

r_gen_gn_df["re_el generation [MWh] rel"] = 0
...
assessment_df["CO2 intensity [g/kWh]"] = assessment_df["CO2 emissions [t]"]/assessment_df["el generation [MWh]"]*1000
#apply RFNBO rules
...
#add colorbar
cbar = plt.cm.ScalarMappable(cmap="viridis_r")
cbar.set_array(assessment_df["CO2 intensity [g/kWh]"])
cbar.set_clim(0, 100)
plt.colorbar(cbar, orientation="vertical", fraction=0.05, pad=0.02)

#insert legend
legend_elements1 =    [
                    Patch(facecolor="limegreen", edgecolor='white', linewidth=2, label='CO2 intensity > 64.8 g/kWh'),
                    Patch(facecolor="limegreen", edgecolor='red', linewidth=8, label='CO2 intensity < 64.8 g/kWh')
                    ]
